...
public class MemberApiController {
    ...
    /**
     * 수정 API
     * 해당 회원 정보는 부분 업데이트이기 때문에, PATCH나 POST를 이용하는 것이 맞다.
     * PUT은 전체 업데이트를 사용할 때 한다.
     */
     ...
     * - 실무에서는 같은 엔티티에 대해 API가 용도에 따라 다양하게 만들어지는데, 한 엔티티에 각각의 API를 위한 프레젠테이션
     * 응답 로직을 담기는 어렵다.
     * - 엔티티가 변경되면 API 스펙이 변한다.
     ...
     */
    // 조회 V1: 안 좋은 버전, 모든 엔티티가 노출, @JsonIgnore -> 이건 정말 최악, api가 이거 하나인가! 화면에
    // 종속적이지 마라!
